...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
    ...
    @Query("SELECT new pl.kodokan.fcp.server.customer.dto.FilteredCustomersDTO(u.id,ud.firstName,ud.lastName) FROM Customer u, UserData ud WHERE ud.firstName LIKE %:firstName% AND ud.lastName LIKE %:lastName% AND CAST(u.cardId as text) LIKE %:cardID% AND CAST(ud.phone as text) LIKE %:phone%")
