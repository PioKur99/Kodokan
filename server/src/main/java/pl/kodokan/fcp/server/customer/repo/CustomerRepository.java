...
public interface CustomerRepository extends JpaRepository<Customer, Long> {
    ...
    @Transactional
    @Query("SELECT u FROM Customer u WHERE LOWER(u.userData.firstName) LIKE %LOWER(:firstName)% AND LOWER(u.userData.lastName) LIKE %LOWER(:lastName)% AND COALESCE(CAST(u.clubCard.id as text),'') LIKE %:cardID% AND COALESCE(CAST(u.userData.phone as text),'') LIKE %:phone% AND (:cardState = '' OR CAST(u.clubCard.state as text) = :cardState)")
